...
MACI 的全名為 Minimum Anti-collusion Infrastructure，最初由 *Vitalik* 在[這篇貼文](https://ethresear.ch/t/minimal-anti-collusion-infrastructure/5413)提出，想法源自解決在區塊鏈上[共謀](https://vitalik.ca/general/2019/04/03/collusion.html)（中文版詳見[此](https://cla2srbchi.medium.com/%E7%BF%BB%E8%AD%AF-vitalik-on-collusion-%E8%AB%96%E5%85%B1%E8%AC%80-8ecd4e012266)）的問題，後續主要由 *BarryWhiteHat* 與 *Kendrick Tan, Kobi Gurkan* 和 *Koh Wei Jie* 等人共同開發。
...
任何人都可以從 etherscan 上直接看見我在鏈上投票的[結果](https://goerli.etherscan.io/tx/0xb6cd91327376b63b8a2b1227ccc8b6a6b3ccc4141e5c4f303b7b80618dd22b86)，就是投給 *John*。
...
2. 在 sign-up 階段，系統會隨機產生一組 EdDSA 計算出的公私鑰對給予投票者，投票者需要保存自己的私鑰（privKey），並將公鑰（pubKey）傳到合約上進行註冊。**合約中會有一棵 Merkle Tree 紀錄整個投票者的註冊狀態、投票權重等（詳細可見圖X）**。
...
4. 投票時，投票者可以做兩種行為（這些行為都可以透過 `publishMessage()` 更新在合約中）：
    ...
    圖Y、來自 Vitalik 在 [ethresear 中提案](https://ethresear.ch/t/minimal-anti-collusion-infrastructure/5413)的內容。
...
更詳盡的系統設計圖可見圖Z，並參見[此影片](https://youtu.be/sKuNj_IQVYI)。
